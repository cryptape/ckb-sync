...
indexer_health_check() {
  # indexer_health_check <port> [timeout_seconds]
  local port="$1"
  local timeout="${2:-120}"

  echo "==> Indexer health check: port=${port}, timeout=${timeout}s"

  # Check LISTEN status
  if ! wait_listen "${port}" "${timeout}"; then
    echo "  ✗ port ${port} not listening (timeout ${timeout}s)"
    return 1
  fi

  # Retry until indexer responds
  local tries=$((timeout / 2))
  local resp
  for ((i=1; i<=tries; i++)); do
    resp=$(curl -sS -X POST -H "Content-Type: application/json" \
      -d '{"id":1,"jsonrpc":"2.0","method":"get_indexer_tip","params":[]}' \
  ...
  echo "  ✗ indexer check failed"
  echo "    last response: $resp"
  return 1
}

echo "------------------------------------------------------------"
indexer_health_check 8114 120 || true
indexer_health_check 8124 120 || true
echo
